...
Dokumen ini berisi panduan lengkap mengenai strategi pengujian (**Unit Testing**, **Widget Testing**, dan **Integration Testing**) pada aplikasi Flutter **Tim Lima FE**, mencakup skenario _Happy Path_ dan _Unhappy Path_, serta strategi optimasi CI/CD pada GitHub Actions.
...
- **Terisolasi & Deterministik**: Bebas dari risiko _flaky test_ akibat port network terblokir atau delay server.
...
| Tipe Tes        | Target Module                      | Skenario _Happy Path_                                                                                                   | Skenario _Unhappy Path_                                                                                                                               |
| :-------------- | :--------------------------------- | :---------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Unit Test**   | `UserModel`                        | Parser JSON lengkap dari backend DTO menghasilkan objek `UserModel` yang valid.                                         | Mengolah Map JSON kosong/null tanpa terjadi _crash_ (menghasilkan default fallback value).                                                            |
...
| **Unit Test**   | `AuthNotifier` (Riverpod)          | Update state `setEmail`, `setPassword`, `setRole`, dan `togglePasswordVisibility` merubah state secara konsisten.       | Form submit dengan email/password kosong memicu error _"Email and password are required"_. Password mismatch memicu error _"Passwords do not match"_. |
...
Skenario pengujian integrasi _End-to-End_ (E2E) dirancang untuk memverifikasi alur lengkap pengguna:
...
2. **Token Blacklist pada Logout**:
   ...
   - Token dimasukkan ke _blacklist_ Redis oleh backend dan dihapus dari `FlutterSecureStorage` lokal.
