...
void main() {
  group('Customer Orders & Refunds Widget Tests', () {
    ...
    testWidgets('Happy Path: CustomerOrdersPage renders orders list and status badges', (tester) async {
      final sampleOrders = [
        CustomerOrderModel(
          id: 'ORD-1001-TEST',
          customerId: 'usr-001',
    ...
    testWidgets('Unhappy Path: CustomerOrdersPage renders empty state when orders list is empty', (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            customerOrdersProvider.overrideWith(
      ...
      expect(find.text('Your ticket booking history will appear here once you make an order.'), findsOneWidget);
    });

    // ==================== CUSTOMER REFUNDS PAGE ====================
    testWidgets('Happy Path: CustomerRefundsPage renders refund requests and status badge', (tester) async {
      final sampleRefunds = [
        CustomerRefundModel(
          id: 'REF-5001-TEST',
          ticketId: 'tkt-001',
    ...
    testWidgets('Unhappy Path: CustomerRefundsPage renders empty state when refund requests list is empty', (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            customerRefundsProvider.overrideWith(
      ...
      expect(find.text('You have not submitted any ticket refund requests.'), findsOneWidget);
    });
  });
}
